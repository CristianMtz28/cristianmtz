...
<div class="login-box">
  ...
  <div class="login-box-body" style="border-radius: 15px">
    <p class="login-box-msg">¡Hola Cristian! Inicia sesión para acceder al panel de administrador.</p>

    <form method="POST" action="{{ route('login') }}">
        @csrf
      <div class="form-group {{ $errors->has('email') ? ' is-invalid' : '' }} has-feedback">
        <input type="email" class="form-control" placeholder="Correo" name="email" value="{{ old('email') }}" required autofocus><br>
        @if ($errors->has('email'))
    ...
    @endif
      ...
      <div class="row">
        ...
        <!-- /.col -->
        <div class="col-xs-12">
          <button type="submit" class="btn btn-primary btn-block btn-flat">Iniciar Sesión</button> <br>
        </div>
        <!-- /.col -->
      </div>
    </form>
